...
#include <string>
// PS: Use Static Link !!
// Postgres imaga do not have GLIBCXX_3.4.21
// g++ judge_psql.cpp -static-libgcc -o judge_psql.out
...
int main(int argc, char *argv[])
{
    ...
    else if (pid == 0)
    {
        ...
        //int newstdin = open("./searchTable.sql", O_RDONLY, 0644);
        int newstdout = open("./result.log", O_WRONLY | O_TRUNC, 0644);
        setProcessLimit(timeLimit, memotyLimit);
        if (newstdout != -1)
        {
            dup2(newstdout, fileno(stdout));
            char *commands[] = {"psql", "-f", "./searchTable.sql", nullptr};
